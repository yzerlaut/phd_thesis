...
(org-add-link-type "citetext"   (defun follow-citet (name)     "Open bibliography and jump to appropriate entry.        The document must contain ibliography{filename} somewhere        for this to work"       (find-file-other-window        (save-excursion          (beginning-of-buffer)          (save-match-data            (re-search-forward "\\\\bibliography{\\([^}]+\\)}")            (concat (match-string 1) ".bib"))))       (beginning-of-buffer)       (search-forward name))     (defun export-citet (path desc format)       "Export [[citet:cohen93]] as \citet{cohen93} in LaTeX."       (if (eq format 'latex)           (if (or (not desc) (equal 0 (search "citet:" desc)))               (format "\\citet{%s}" path)             (format "\\citet[%s]{%s}" desc path)))))
(org-add-link-type "cite"   (defun follow-citep (name)     "Open bibliography and jump to appropriate entry.        The document must contain ibliography{filename} somewhere        for this to work"       (find-file-other-window        (save-excursion          (beginning-of-buffer)          (save-match-data            (re-search-forward "\\\\bibliography{\\([^}]+\\)}")            (concat (match-string 1) ".bib"))))       (beginning-of-buffer)       (search-forward name))     (defun export-citep (path desc format)       "Export [[citep:cohen93]] as \citep{cohen93} in LaTeX."       (if (eq format 'latex)           (if (or (not desc) (equal 0 (search "citep:" desc)))               (format "\\citep{%s}" path)             (format "\\citep[%s]{%s}" desc path)))))
